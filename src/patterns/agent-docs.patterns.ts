 ...
 *
 * These paths are useful in two ways: 1. As oxfmt overrides — apply relaxed markdown formatting to agent docs
 * 2. As a shared constant for eslint/markdownlint configs — consumers can import this array to disable
 * structural markdown rules for agent files
 *
 * Sources: - GitHub Copilot: docs.github.com/copilot/customizing-copilot - Cursor:
 * docs.cursor.com/context/rules - Windsurf: windsurf.com docs + .windsurf/rules - Claude Code: CLAUDE.md
 * convention - Cline: .clinerules convention - Gemini: GEMINI.md convention (supported by GitHub Copilot
 * CLI)
 */
 ...
 *
 * Filters out non-markdown files (.mdc, extensionless) that oxfmt's markdown override wouldn't match anyway.
 * Use this for the `files` array in oxfmt overrides.
 */
